...
from util import GaussianFilter_2D_jax, getWaveletTransforms_2D_jax_scale, legend_without_duplicate_labels
import arviz as az
...
n,m = cam.shape
#py_W, py_Ws, scaling_vec = getWaveletTransforms_2D_jax_scale(n,m,wavelet_type = "haar", level = 6, weight = 0.6)
py_W, py_Ws, scaling_vec = getWaveletTransforms_2D_jax_scale(n,m,wavelet_type = "db4", level = 6, weight = 0.6)
...
for beta in betavals:
    ...
    for il in lamfac:
        ...
        save_xtra = os.path.join("SIMULATION","WAVELET_2D", f"lam{il}beta{beta}weighted")
        os.makedirs(save_xtra, exist_ok=True)
        #Save the original and blurred images to every subfolder
        filename_orig = "cam_original.pdf"
        fig_orig_path = os.path.join(save_xtra,filename_orig)
        fig_orig.savefig(fig_orig_path, bbox_inches='tight')
        filename_blur = "cam_blurred.pdf"
        fig_blur_path = os.path.join(save_xtra,filename_blur)
        fig_mod.savefig(fig_blur_path, bbox_inches='tight')
        ...
        for a in range(len(method_names)):
            filename = f"mean_rec_{method_names[a]}.pdf"
            fig_path = os.path.join(save_xtra,filename)
            fig_mean,ax_mean = plt.subplots()
            ax_mean.imshow(clamp(py_Ws(jnp.reshape(scaling_vec,(-1,1))*jnp.mean(x_out[a,:,:,:],axis=0))), cmap = "gray", vmin=0, vmax=1)
            fig_mean.savefig(fig_path)
            plt.pause(1)
            plt.clf()
            time.sleep(0.1)
        ...
        for a in range(len(method_names)):
            ...
            for i in range(thinned_steps):
               ...
               x_out_fin.append(py_Ws(scaling_vec*jnp.array(x_out[a,i,:,0])))
...
for beta in betavals:
    for il in lamfac:
        for a in range(len(method_names)):
            ...
            filename_cred = f"cred_{cred_upper - cred_lower}_{method_names[a]}.pdf"
            ...
            filename_acf = f"acf_lag_{chosen_lag}_{method_names[a]}.pdf"
...
filename_acfm = f"acf_mean_{max_lag}.pdf"
