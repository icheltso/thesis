...
for i in range(n_methods):
    ...
    axes[i].scatter(range(len(y_test)), y_test_adjusted[:,0], color='red', marker='x', label="True Labels")
...
for i in range(n_methods):
    ...
    axes[i].set_title("Density of Posterior Probabilities")
    axes[i].legend()
plt.tight_layout()
plt.show()
...
for i in range(n_methods):
    ...
    axes[i].set_xticklabels(feature_names, rotation=75)
    axes[i].set_xlabel("Feature Index")
    axes[i].set_ylabel("Standard Deviation")
    axes[i].legend()
    axes[i].set_title("Comparison of Data vs. Posterior Uncertainty")
plt.tight_layout()
plt.show()
