...
save_xtra = os.path.join("SIMULATION","BINARY","CANCER")
...
file_path = os.path.join(save_xtra, "breast_cancer_data.npz")
...
if reload_var == True or not all(var in globals() for var in ["X_train", "X_test", "y_train", "y_test"]):
    ...
    if os.path.exists(file_path):
        ...
        print("Wisconsin Breast Cancer dataset loaded from local file.")
    
    else:
        # Load the dataset
        data = load_breast_cancer()
        X_full, y_full = data.data, data.target  # Features and labels
        
        # Save dataset locally
        np.savez_compressed(file_path, X=X_full, y=y_full)
        print("Wisconsin Breast Cancer dataset downloaded and saved locally.")

    # Convert labels to {-1, 1} for logistic loss
    y_full_binary = 2 * y_full.reshape(-1, 1) - 1

    # Standardize features
    scaler = StandardScaler()
    X_full_scaled = scaler.fit_transform(X_full)
    
    total_samples = X_full_scaled.shape[0]
    train_size = int(0.8 * total_samples)
    test_size = total_samples - train_size  # Remaining data

    # Split into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(
        X_full_scaled, y_full_binary, train_size=train_size, test_size=test_size
    )
...
#taus = [4/Lf,0.0001,0.0001,0.0001]
#taus = [1/(10*Lf)]*len(method_base)
...
for i in range(n_methods):
    mean_true_ALL.append(mean_predictions[i][y_test[:,0] == 1])
    mean_false_ALL.append(mean_predictions[i][y_test[:,0] == -1])
...
for i in range(n_methods):
    plt.scatter(mean_predictions[i], var_predictions[i], label = method_base[i])
    plt.ylim(0,ylimit)
    plt.xlabel("Mean Posterior Probability")
    plt.ylabel("Variance of Posterior Probability")
    plt.title("Uncertainty Across Test Cases")
    plt.legend()
    plt.show()
...
for i in range(n_methods):
    ...
    plt.hist(mean_predictions[i], bins=20, alpha=0.7)
    plt.xlabel("Mean Posterior Probability")
    plt.ylabel("Number of Test Cases")
    plt.title(ttl_str)
    plt.show()
    
    ttl_str = method_base[i] + " - Distribution of Mean Posterior Probabilities:True vs False Cases"
    plt.hist(mean_true_ALL[i], bins=20, alpha=0.5, label="True Class Cases")
    plt.hist(mean_false_ALL[i], bins=20, alpha=0.5, label="False Class Cases")
    plt.xlabel("Mean Posterior Probability")
    plt.ylabel("Number of Test Cases")
    plt.title(ttl_str)
    plt.show()
...
for i in range(n_methods):
    correct_uncertainty.append(uncertainty_range[i][y_test_adjusted[:,0] == binary_predictions[i]])  # Uncertainty for correct cases
    incorrect_uncertainty.append(uncertainty_range[i][y_test_adjusted[:,0] != binary_predictions[i]])  # Uncertainty for incorrect cases

"Plot histograms for uncertainty in predictions"
for i in range(n_methods): 
    plt.hist(correct_uncertainty[i], alpha=0.5, label="Correctly Classified", bins=20)
    plt.hist(incorrect_uncertainty[i], alpha=0.5, label="Incorrectly Classified", bins=20)
    plt.xlabel("95% CI Width (Uncertainty)")
    plt.ylabel("Number of Test Cases")
    plt.title("Uncertainty in Correct vs Incorrect Classifications")
    plt.legend()
    plt.show()

"percentile plots"
for i in range(n_methods):
    plt.scatter(range(len(y_test)), mean_predictions[i], label="Mean Posterior Probability", alpha=0.6)
    plt.fill_between(range(len(y_test)), percentile_probs_low[i], percentile_probs_hi[i], color='b', alpha=0.3, label="95% CI")
    plt.scatter(range(len(y_test)), y_test_adjusted[:,0], color='red', marker='x', label="True Labels")
    plt.xlabel("Test Case Index")
    plt.ylabel("Posterior Probability")
    plt.title("Mean Posterior Probability with 95% Credible Interval")
    plt.legend()
    plt.show()
    
"kde plots for percentiles"
for i in range(n_methods):
    sns.kdeplot(mean_predictions[i], label="Mean Posterior", shade=True)
    sns.kdeplot(percentile_probs_low[i], label="2.5% Percentile", linestyle="--")
    sns.kdeplot(percentile_probs_hi[i], label="97.5% Percentile", linestyle="--")
    plt.xlabel("Posterior Probability")
    plt.title("Density of Posterior Probabilities")
    plt.legend()
    plt.show()
...
for i in range(n_methods):
    plt.bar(feature_names, feature_std, alpha=0.5, label="Data Std")
    plt.bar(feature_names, std_image[i], alpha=0.5, label="Posterior Std")
    plt.xticks(rotation=90)  # Rotate feature names for readability
    plt.xlabel("Feature Index")
    plt.ylabel("Standard Deviation")
    plt.legend()
    plt.title("Comparison of Data vs. Posterior Uncertainty")
    plt.show()
...
for i in range(n_methods):
    ...
    save_dist_str = "barplot_" + method_base[i] + ".pdf"
    fig_path_bar = os.path.join(save_xtra, save_dist_str)
    ...
    plt.figure(figsize=(12, 6))
    plt.bar(range(len(mean_image[i])), mean_image[i], tick_label=feature_names)
    ...
    save_dist_str = "Mean_vs_MAP_" + method_base[i] + ".pdf"
    ...
    plt.figure(figsize=(12, 6))
    plt.bar(range(len(abs_diff)), abs_diff, tick_label=feature_names)
    ...
    ttl_str = method_base[i] + ' - Log-Difference Between 95th and 5th Quantiles'
    ...
    plt.figure(figsize=(12, 6))
    plt.bar(range(len(quantile_diff_method)), quantile_diff_method, tick_label=feature_names)
    plt.xticks(rotation=90)
    plt.ylabel("Log |95th - 5th Quantile|")
    plt.title(ttl_str)
    plt.grid()
